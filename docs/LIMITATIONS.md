...
restart. ADK ships `DatabaseSessionService`; swapping it into
`thread-runner.ts` covers agent history. The thread registry and replay buffer
are separate and would each need a store.
...
guesses one reads that feed. Production needs a real identity on the connection.
...
had, and the server never forgets one ([L3](#l3)).
