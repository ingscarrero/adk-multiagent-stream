...
| Server | Vitest (node) | 67 | Adapter mapping, real HTTP/SSE, concurrency, reconnect, cancellation, the two memory bounds, follow-up turns, the confirmation gate and the durable transcript |
...
271 in `pnpm test`, 81 in `pnpm test:e2e`, and 7 eval cases that run both as a
